...
#define restricted_branch_speculation_start()				\
do {									\
	u64 val = x86_spec_ctrl_base | SPEC_CTRL_IBRS;			\
									\
	if (ibrs_enabled)						\
...
#define restricted_branch_speculation_end()				\
do {									\
	u64 val = x86_spec_ctrl_base;					\
									\
	if (ibrs_enabled)						\
...
{
	if (static_branch_likely_init_false(&mds_user_clear))
...
{
	if (static_branch_likely_init_false(&mds_idle_clear))
		mds_clear_cpu_buffers();
}
